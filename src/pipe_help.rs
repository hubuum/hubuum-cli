...
pub fn verb_summaries() -> &'static [VerbSummary] {
    &[
        ...
        VerbSummary {
            names: "A",
            topic: "group",
            summary: "Aggregate grouped rows or a complete collection.",
        },
        VerbSummary {
            names: "U",
            topic: "group",
...
pub fn topic_help(topic: &str) -> Option<&'static str> {
    match topic {
        ...
        "group" => Some(
            "Grouping and aggregation stages:\n  | G <field> [AS alias] - group rows by one selector, optionally naming the output column.\n  | A <aggregate> [AS alias] - add one aggregate to each group.\n  | A GLOBAL <aggregate> [AS alias], ... - consume Empty, Rows, or Values and return one row.\n  | Z - collapse groups to one summary row per group.\n  | U <array-field> - unroll row arrays before G or visible summary arrays after G.\n\nAggregates are count, count(field), count_distinct(field), sum(field), avg(field), min(field), max(field), first(field), and last(field). count(field) counts every non-null fanout match. count_distinct(field) excludes missing and null and uses the same JSON equality as D. first(field) and last(field) skip missing and null and use current row and selector order; sort before G or A GLOBAL for reproducible boundaries. Global output names must be unique. Use --all when a paginated command must aggregate every matching result.\n\nAfter G, filters, truthiness, key/value search, projection, sorting, and unroll operate on visible group and aggregate aliases. Filters retain or remove whole groups; hidden member rows never change. Grouped C emits each summary with its member count. Use Z before A GLOBAL when aggregating visible group summaries.\n\nExamples:\n  object list --class Hosts | G os_version AS \"OS Version\" | A count AS Hosts\n  object list --class Hosts --all | A GLOBAL count AS Hosts, count(data.owner) AS Owned, count_distinct(os_version) AS Versions\n  task events <task-id> --all | S created_at asc AS datetime | A GLOBAL first(created_at) AS First, last(created_at) AS Last\n  audit list --all | S occurred_at asc AS datetime | G action | A first(occurred_at) AS First | A last(occurred_at) AS Last | Z\n  object list --class Hosts | G data.network.interfaces[*].ipv4 AS IPv4 | C\n  object list --class Hosts | U data.network.interfaces | P Name ipv4 mac",
        ),
        "selectors" => Some(
